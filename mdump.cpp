...
/*!
    \file mdump.h
		\version \$Id: mdump.cpp,v 1.14 2004/03/08 07:56:51 jcsston Exp $
    \brief Crash Catcher
...
{	
	...
	std::basic_string<TCHAR> strStackDump;
	strStackDump = StackDump(pExceptionInfo->ContextRecord->Ebp, pExceptionInfo->ContextRecord->Eip);
	
	GetModuleFileName(g_hmodThisDll, szScratchPad, 255);
	if (_tcsstr(strStackDump.c_str(), szScratchPad) == NULL) {
		// Not a MatroskaProp.dll crash
		return EXCEPTION_CONTINUE_SEARCH;
	}
	strDebugMsg += strStackDump;
	...
	myCrash.filename = _T("shell_ext_crash_dump.zip");
	
	// ask the user if they want to save a dump file			
	int dlgChoice = DialogBoxParam(g_hmodThisDll, MAKEINTRESOURCE(IDD_DIALOG_CRASH), hParent, CrashDlgProc, (LPARAM)&myCrash);
	...
	if (dlgChoice == IDOK) {	
		...
		if (hDll) {
			...
			if (pDump) {		
				...
				// Also having the build number it really helpfull
				_sntprintf(szScratchPad, 255, _T(" Build %i"), g_MyBuildNumber);
				strDumpPath += szScratchPad;
		...
		_sntprintf(szScratchPad, 255, _T("Saved dump file to '%s'.\nPlease send this file to me 'jcsston' at matroskaprop@jory.info."), myCrash.filename.c_str());
...
std::basic_string<TCHAR> CrashCatcher::ProcessExceptionRecord(EXCEPTION_RECORD *pExceptionRecord) {
	...
	while (pExceptionRecord != NULL) {
		...
		} else if (pExceptionRecord->ExceptionCode == 0xC06D007E) {
			//while (true);
			_sntprintf(szScratchPad, 255, _T("Module not found, at Address %p\r\n"), pExceptionRecord->ExceptionCode, pExceptionRecord->ExceptionAddress);
			strDebugMsg += szScratchPad;
...
LPCTSTR CrashCatcher::GetAddressInfo(const BYTE* caller, HINSTANCE hInstance)
{
	static TCHAR AddressInfo[256] = { 0 };
	static DWORD len = 0;
  	
	DWORD relativeAddress = caller - (BYTE*)hInstance;
	
	len = _sntprintf(AddressInfo, 255, _T("%08X : "), caller);	
	
	GetModuleFileName(hInstance, AddressInfo+len, 255-len);	

	_tcscat(AddressInfo, _T(" \r\n"));
