...
/*!
    \file ZipArchive.cpp
		\version \$Id: ZipArchive.cpp,v 1.6 2004/03/06 09:10:05 jcsston Exp $
...
ZipArchiveWriter::ZipArchiveWriter(const wchar_t *outputArchive) {
	size_t sourceStrLen = wcslen(outputArchive)+1;
	m_OutputFilename.resize(sourceStrLen);
	for (size_t cw1 = 0; cw1 < sourceStrLen; cw1++) 
		m_OutputFilename[cw1] = (char)outputArchive[cw1];	
...
void ZipArchiveWriter::AddFile(const wchar_t *sourceFilename) {
	...
	size_t sourceStrLen = wcslen(sourceFilename)+1;
	sourceStr.resize(sourceStrLen);
	for (size_t cw1 = 0; cw1 < sourceStrLen; cw1++) 
		sourceStr[cw1] = (char)sourceFilename[cw1];	
...
void ZipArchiveWriter::AddFile(const std::string &newFile, const unsigned char *data, unsigned long size) {
	...
	try {
		...
		if (ret == Z_OK) {
			...
			uLong compressedBufferSize = (uLong)((float)size * 1.25);
  		...
  		fileHeader.compressed_size = zlibStream.total_out-2;
				...
				compressedBufferSize = (uLong)((float)compressedBufferSize * 1.5);
				compressedBuffer = new Byte[compressedBufferSize];
				assert(compressedBuffer != NULL);
		...
		// zlib must have failed

		fileHeader.uncompressed_size = size;
		// it's uncompressed, because zlib failed to load
		fileHeader.compressed_size = size;
		
		// without zlib we cannot calc the CRC32
		fileHeader.crc32 = 0;
  	...
  	fwrite(data, 1, fileHeader.compressed_size, m_OutputFile);
	}
