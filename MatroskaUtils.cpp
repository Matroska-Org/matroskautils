						...
						if ((*SeekHead)[Index0]->Generic().GlobalId == KaxSeek::ClassInfos.GlobalId) {
							...
							for (Index1 = 0; Index1 < SeekEntry.ListSize(); Index1++) {
								if (SeekEntry[Index1]->Generic().GlobalId == KaxSeekID::ClassInfos.GlobalId) {
									...
									} else if ((*id == KaxAttachments::ClassInfos.GlobalId) && m_parseAttachments) {
		...
		if (EbmlId(*l2) == KaxTrackEntry::ClassInfos.GlobalId) {
			...
			while (l3 != NULL) {
				...
				if (EbmlId(*l3) == KaxTrackNumber::ClassInfos.GlobalId) {
					...
					if (!strcmp(std::string(CodecID).c_str(), "V_MS/VFW/FOURCC")
						||!strcmp(std::string(CodecID).c_str(), "A_MS/ACM"))
						vfw_codec = true;
					else
						vfw_codec = false;
					...
					if (vfw_codec && (c_priv.GetSize() >= sizeof(vfw_BITMAPINFOHEADER)) && (matroskaTrack->GetTrackType() == track_video)) {
						vfw_BITMAPINFOHEADER *bih = (vfw_BITMAPINFOHEADER *)c_priv.GetBuffer();
						
						const char *fcc_name = MatroskaVideoTrackInfo::GetFourCCName((char *)&bih->bi_compression);
						JString fccCode;
						if (fcc_name != NULL) {
							fccCode = fcc_name;
							fccCode += " ";
						}
						fccCode += JString::Format("(%c%c%c%c)", ((char *)&bih->bi_compression)[0], ((char *)&bih->bi_compression)[1], ((char *)&bih->bi_compression)[2], ((char *)&bih->bi_compression)[3]);
						
						matroskaTrack->m_CodecOldID = fccCode.mb_str();

					}else if (vfw_codec && (c_priv.GetSize() >= sizeof(acm_WAVEFORMATEX)) && (matroskaTrack->GetTrackType() == track_audio)) {
						acm_WAVEFORMATEX *bih = (acm_WAVEFORMATEX *)c_priv.GetBuffer();
						const char *acm_name = MatroskaAudioTrackInfo::GetWaveFormatName(*((uint16 *)bih->wFormatTag));
						JString acm_wav;
						if (acm_name != NULL) {
							acm_wav = acm_name;
							acm_wav += " ";
...
MatroskaTrackInfo *MatroskaInfoParser::GetTrackInfo(int trackNumber){
	TRACE("MatroskaInfoParser::GetTrackInfo()");	
